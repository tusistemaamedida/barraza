...
routes.post("/", async (req, resp) => {
  let { body } = req;
  try {
    let data = await ctrProducts.save(body);
...
routes.post("/upload", async (req, resp) => {
  ...
  try {
    let { mimetype } = req.files.file;
    let extencion = mimetype.split("/");
    let nameImage = `${uuid4()}.${extencion[1]}`;
    fs.writeFileSync(`./images/${nameImage}`, req.files.file.data);
    resp
      .status(200)
      .send(
        util.getSuccessMsg({ url: `http://localhost:3100/images/${nameImage}` })
      );
  } catch (error) {
    resp.status(500).send(util.getErrorMsg(error));
  }
